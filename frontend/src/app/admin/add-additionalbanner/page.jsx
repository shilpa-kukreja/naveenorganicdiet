...
const API_URL = process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL}/api/additionalbanner`;
...
const AdditionalBanner = () => {
  ...
  const fetchBanners = async () => {
    try {
      ...
      setBanners(response.data.additionalbanners || []);
    } catch (error) {
  ...
  // Update banner status - FIXED: Changed to match backend route
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      // Since your backend only updates image, not status, you might need to adjust
      // If you want to toggle status, you need a separate endpoint
      // For now, I'll create a status toggle endpoint in backend
      showNotification('Status toggle not implemented yet', 'error');
  ...
  // Update banner image - FIXED: Changed to match backend route
  const handleUpdateImage = async (id) => {
    ...
    try {
      ...
      await axios.put(`${API_URL}/update-status/${id}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      });
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Additional Banners</h1>
            <p className="text-gray-600 mt-2">Manage your additional banner images</p>
          </div>
          <button
